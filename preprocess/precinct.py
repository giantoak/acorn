...
if __name__ == '__main__':
    ...
    df_new = pd.concat([df, namesdf], names=(df.columns + 'name'),  axis=1)
    df_new['full_name'] = df_new[0] + ',' + df_new['State']
    df_new.rename({0: 'name'}, axis=1, inplace=True)
    df_new.to_csv('crime_scrape2013_fullnames.csv', index=False)
